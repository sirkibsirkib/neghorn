...
enum StateRuleInstruction {
  // TODO
}
struct StateRule {
  instructions: StateRuleInstruction,
}

struct State {
  state_rules: Vec<StateRule>,
  pos: HashMap<TypeId, chunk_arena::ChunkArena>,
}
    ...
    match tid {
      ...
      _ => {
        if depth_to_go == 0 {
          ...
          let fields = self.type_fields.get(&tid).ok_or(TidSizeErr::DependsOnSizeOfUnknown(tid))?;
          fields
            .iter()
            .fold(Ok(0), |acc, &field| Ok(acc? + self.compute_size_of(field, depth_to_go - 1)?))
        }
      }
    }
  }
